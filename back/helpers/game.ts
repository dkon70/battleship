      ...
      if (enemyField[i].coordinates[j].x === coordinates.x && enemyField[i].coordinates[j].y === coordinates.y) {
        ...
        if (checkDestroy(obj)) {
          ...
          if (checkWin(enemyField)) {
            ...
            return ["finish", JSON.stringify({ type: "attack", data: JSON.stringify({ position: { x: coordinates.x, y: coordinates.y }, currentPlayer: playerID, status: "shot" }) }), JSON.stringify({ type: "attack", data: JSON.stringify({ position: { x: coordinates.x, y: coordinates.y }, currentPlayer: playerID, status: "killed" }) }), JSON.stringify({ type: "finish", data: JSON.stringify({ winPlayer: playerID }) })];
          } else {
